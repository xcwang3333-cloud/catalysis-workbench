...
v0.4 implementation is active. The shared constrained peak-fitting foundation was reviewed and merged through Issue #75 / PR #76 at `b6f428d96df9950373c17e5de487ac4113a2aacc`. The XPS preparation layer was then reviewed and merged through Issue #79 / PR #80 at `a13dbd541b299f79d83e47f079c4638b082a8061`, adding explicit binding-energy/eV semantics, caller-supplied additive energy correction, measured-point region preparation, direction-safe linear background, and an independently implemented Shirley fixed-point background with explicit convergence/failure state. The next v0.4 scientific stage is constrained XPS peak/doublet fitting as a domain consumer of the shared fitter; publication plotting remains later.
...
The reviewed XPS preparation API is exported from `catalysis_workbench.experimental.characterization`.
...
- `catalysis_workbench.experimental.characterization`: XRD, Raman, FTIR/ATR-FTIR, TGA/DTG/TPR/TPD, basic gas-sorption, ICP/elemental-composition, and XPS preparation; XPS public state includes validation, explicit additive binding-energy correction, measured-point region preparation, immutable background results, and linear/Shirley backgrounds; existing characterization publication plotting remains lazy and XPS plotting is not yet implemented.
...
- Characterization: XRD, Raman, FTIR/ATR-FTIR, TGA/DTG/TPR/TPD, basic gas-sorption, and ICP/composition are implemented and released in v0.3. The shared constrained peak-fitting foundation and XPS semantics/energy-correction/region/background-preparation layer are now implemented on `main` as the first two v0.4 scientific blocks. Constrained XPS fitting is next; XPS plotting and quantitative BET remain subsequent v0.4 work. XAS remains staged later.
...
v0.4 architecture was defined through Issue #73 / PR #74. Shared constrained peak fitting completed Issue #75 / PR #76 with exact-head CI #255, two formal reviews, and squash merge commit `b6f428d96df9950373c17e5de487ac4113a2aacc`. XPS preparation completed Issue #79 / PR #80 with exact-head CI #261, formal reviews `5008700786` and `5008706395`, and squash merge commit `a13dbd541b299f79d83e47f079c4638b082a8061`. Runtime/distribution version remains `0.3.0`; these development checkpoints do not authorize a v0.4 release/tag/package publication.
...
See [`docs/MASTER_PLAN.md`](docs/MASTER_PLAN.md) for the project-wide execution model and current checkpoint, [`docs/V0_4_PLAN.md`](docs/V0_4_PLAN.md) for the active v0.4 dependency order, [`docs/PEAK_FITTING.md`](docs/PEAK_FITTING.md) for the shared fitting contract, [`docs/XPS.md`](docs/XPS.md) for the XPS preparation contract, [`docs/V0_3_RELEASING.md`](docs/V0_3_RELEASING.md) for the completed v0.3 release record, and [`docs/ROADMAP.md`](docs/ROADMAP.md) for the long-range release scope.
...
- **v0.4:** implementation active — shared constrained fitting and XPS preparation complete; constrained XPS fitting is next, followed by XPS plotting, EIS, quantitative BET, and product calibration.
