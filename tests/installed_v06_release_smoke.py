"""Unified Gate-A audit for the frozen v0.6 installed wheel.
...
version, and public-API invariants, then reuses the already reviewed v0.5 release audit plus
all reviewed base-environment v0.6 installed-wheel smoke programs as independent
subprocesses so Gate A does not duplicate or weaken their scientific checks.
...
REQUIRED_V06_COMPUTATION_EXPORTS = (
    "VolumetricGrid",
    "ElectronicDOS",
    "DOSTrace",
...
REVIEWED_INSTALLED_SMOKES = (
    "installed_v05_release_smoke.py",
    "installed_dos_smoke.py",
    "installed_bader_smoke.py",
    "installed_correlation_smoke.py",
    "installed_thermodynamics_smoke.py",
    "installed_free_energy_diagram_smoke.py",
    "installed_charge_density_difference_smoke.py",
)
...
def _assert_public_exports() -> None:
    ...
    exported = set(computation.__all__)
    missing = [name for name in REQUIRED_V06_COMPUTATION_EXPORTS if name not in exported]
    assert not missing, f"missing reviewed v0.6 computation exports: {missing!r}"
    for name in REQUIRED_V06_COMPUTATION_EXPORTS:
...
def main() -> None:
    ...
    print("installed v0.6 Gate-A release audit: ok")
