...
def test_analysis_document_is_immutable_deterministic_and_normalizes_legacy_schemas() -> None:
...
def test_analysis_document_data_series_and_processing_change_identity(tmp_path) -> None:
    ...
    processed = AnalysisDocument(
        ...
        title="LSV",
        data_series=(first_series,),
        analysis=LSVAnalysisSpec(common=LSVProcessingSpec(resistance_ohm=1.0)),
    )
    ...
    with pytest.raises(AnalysisDocumentError, match="unknown data_id"):
        AnalysisDocument(
            ...
            analysis=LSVAnalysisSpec(
                overrides={"series-missing": LSVProcessingSpec()}
            ),
        )
