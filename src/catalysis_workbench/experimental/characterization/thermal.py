...
def _temperature_unit(unit: str | None) -> TemperatureUnit:
    if unit is None or not str(unit).strip():
        raise ThermalError(
            "temperature requires an explicit degree-Celsius or kelvin unit"
        )
...
def _normalized_unit(unit: str | None, *, semantic: str) -> str:
    if unit is None or not str(unit).strip():
    ...
    if semantic == "mass_percent" and token in _PERCENT_UNITS:
        return "%"
    raise ThermalError(
        f"{semantic} has incompatible unit {unit!r}; "
        "use '1' for fraction or '%' for percent"
    )


def _tga_y_unit(series: Series, semantic: str) -> str:
    if semantic == "mass":
        return _mass_unit(series.y_axis.unit)
    return _normalized_unit(series.y_axis.unit, semantic=semantic)
...
def _dtg_base_unit(series: Series, source_semantic: str) -> str:
    unit = str(series.y_axis.unit or "").strip()
    if "/" not in unit:
        raise ThermalError("DTG requires a derivative unit with a temperature denominator")
    base, _ = unit.rsplit("/", 1)
    if source_semantic == "mass":
        return _mass_unit(base)
    if source_semantic == "mass_fraction":
        return _normalized_unit(base, semantic="mass_fraction")
    if source_semantic == "mass_percent":
        return _normalized_unit(base, semantic="mass_percent")
    raise ThermalError("unsupported DTG source mass semantic")
...
def validate_dtg_series(series: Series) -> None:
    ...
    if y_name == "massderivative" and sign_mode != "signed":
        raise ThermalError("mass_derivative requires dtg_sign_mode='signed'")
    if y_name == "masslossrate" and sign_mode != "mass_loss_positive":
        raise ThermalError(
            "mass_loss_rate requires dtg_sign_mode='mass_loss_positive'"
        )
    ...
    base_unit = _dtg_base_unit(series, source_semantic)
...
def convert_temperature(
    ...
    *,
    ...
    if is_dtg:
        ...
        y_metadata.update(
            {
                "temperature_denominator_conversion": f"{source_unit}->{target}",
                "source_sha256": source_sha256,
            }
        )
        y_axis = Axis(
            series.y_axis.name,
            unit=_dtg_unit(base_unit, target),
...
def validate_thermal_overlay(
    ...
    *,
    ...
    """Reject overlays with incompatible technique/unit/normalization/sign state."""
...
def measure_thermal_window(
    ...
    *,
    ...
    n_measured = int(np.count_nonzero(measured_mask))
    if n_measured == 0:
        raise ThermalError(
            "thermal window contains no measured temperature points; "
            "boundary interpolation alone is insufficient"
        )
    ...
    area_unit = (
        ...
        else f"{signal_unit}·{temperature_unit}"
    )
...
class ThermalAnnotation:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "color", None if self.color is None else str(self.color))
