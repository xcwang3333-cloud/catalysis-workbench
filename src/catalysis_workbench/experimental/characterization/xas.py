...
def _immutable_float_array(values: Any, *, name: str) -> NDArray[np.float64]:
    ...
    buffer = result.tobytes(order="C")
    frozen = np.frombuffer(buffer, dtype=np.float64, count=result.size)
...
def _validate_real_finite_series(series: Series) -> tuple[NDArray[np.float64], NDArray[np.float64]]:
...
def validate_xas_series(series: Series, *, allow_relative_energy: bool = False) -> None:
    ...
    _validate_real_finite_series(series)
    ...
    if x_name not in allowed_x:
        expected = "energy or energy_relative_to_e0" if allow_relative_energy else "energy"
...
class XANESNormalizationSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.pre_edge, XASWindow) or not isinstance(self.post_edge, XASWindow):
            raise TypeError("pre_edge and post_edge must be XASWindow instances")
        if isinstance(self.pre_edge_order, bool) or not isinstance(self.pre_edge_order, int):
            raise TypeError("pre_edge_order must be an integer")
        if isinstance(self.post_edge_order, bool) or not isinstance(self.post_edge_order, int):
            raise TypeError("post_edge_order must be an integer")
        if self.pre_edge_order < 0 or self.post_edge_order < 0:
            raise XASError("polynomial orders must be non-negative")
        if self.pre_edge_order > 4 or self.post_edge_order > 4:
            raise XASError("v0.5 XANES polynomial orders are limited to 0..4")
...
def _evaluate_polynomial(coefficients: NDArray[np.float64], delta_energy: NDArray[np.float64]) -> NDArray[np.float64]:
...
class XANESNormalizationResult:
    ...
    def __post_init__(self) -> None:
        ...
        pre_coeff = _immutable_float_array(self.pre_edge_coefficients, name="pre_edge_coefficients")
        post_coeff = _immutable_float_array(self.post_edge_coefficients, name="post_edge_coefficients")
        pre_curve = _immutable_float_array(self.pre_edge_curve, name="pre_edge_curve")
        post_curve = _immutable_float_array(self.post_edge_curve, name="post_edge_curve")
        ...
        if not isinstance(self.pre_edge, XASWindow) or not isinstance(self.post_edge, XASWindow):
            raise TypeError("retained XANES windows must be XASWindow instances")
        ...
        if pre_coeff.size != self.pre_edge_order + 1:
            raise XASError("pre-edge coefficient count does not match polynomial order")
        if post_coeff.size != self.post_edge_order + 1:
            raise XASError("post-edge coefficient count does not match polynomial order")
        expected_pre = _evaluate_polynomial(pre_coeff, energy - e0)
        expected_post = _evaluate_polynomial(post_coeff, energy - e0)
        ...
        if not np.allclose(np.asarray(self.normalized.y), expected_norm, rtol=1e-12, atol=1e-12):
        ...
        metadata_digest = self.normalized.metadata.get("xas_source_digest")
        if metadata_digest != str(self.source_digest):
    ...
    def equals(self, other: object) -> bool:
        return (
            ...
            and np.array_equal(self.pre_edge_coefficients, other.pre_edge_coefficients)
            and np.array_equal(self.post_edge_coefficients, other.post_edge_coefficients)
...
def shift_xas_energy(series: Series, shift_ev: float, *, reference: str | None = None) -> Series:
    ...
    metadata["xas_energy_shift_ev"] = float(metadata.get("xas_energy_shift_ev", 0.0)) + shift
    return Series(
...
def normalize_xanes(series: Series, spec: XANESNormalizationSpec) -> XANESNormalizationResult:
    ...
    energy, mu = _validate_real_finite_series(series)
    ...
    pre_coeff, _ = _fit_centered_polynomial(
        energy,
        mu,
    ...
    post_coeff, _ = _fit_centered_polynomial(
        energy,
        mu,
    ...
    delta = energy - spec.e0_ev
    pre_curve = _evaluate_polynomial(pre_coeff, delta)
    post_curve = _evaluate_polynomial(post_coeff, delta)
