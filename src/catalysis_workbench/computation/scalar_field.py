...
def _frozen_float_array(values: Any, *, name: str, ndim: int) -> NDArray[np.float64]:
    ...
    frozen = np.frombuffer(contiguous.tobytes(order="C"), dtype=np.float64).reshape(contiguous.shape)
...
def _grid_index(index: Sequence[int], *, grid_shape: tuple[int, int, int]) -> tuple[int, int, int]:
    if len(index) != 3:
    ...
    for axis, (raw, size) in enumerate(zip(index, grid_shape, strict=True)):
        ...
        if value < 0 or value >= size:
            raise ScalarFieldError(f"grid index axis {axis}={value} is outside [0, {size})")
...
class ScalarField:
    ...
    def __post_init__(self) -> None:
        ...
        if self.structure.lattice_angstrom is None or not all(self.structure.pbc):
            raise ScalarFieldError("scalar fields require a fully periodic structure with an explicit lattice")
        ...
        registration_id = _nonblank(self.registration_id, name="registration_id", optional=True)
        ...
        for text in (self.structure.digest, field_kind, value_unit, source_type, source_key, source_digest, registration_id):
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, ScalarField) and self.digest == other.digest and self.structure == other.structure and np.array_equal(self.values, other.values)
...
class ScalarFieldSlice:
    ...
    def __post_init__(self) -> None:
        source_field_digest = str(_nonblank(self.source_field_digest, name="source_field_digest"))
        structure_digest = str(_nonblank(self.structure_digest, name="structure_digest"))
        lattice = _frozen_lattice(self.lattice_angstrom)
        shape = tuple(int(value) for value in self.grid_shape)
        if len(shape) != 3 or any(value <= 0 for value in shape):
            raise ScalarFieldError("grid_shape must contain three positive integers")
        field_kind = str(_nonblank(self.field_kind, name="field_kind"))
        value_unit = str(_nonblank(self.value_unit, name="value_unit"))
        registration_id = _nonblank(self.registration_id, name="registration_id", optional=True)
        if isinstance(self.axis, bool) or not isinstance(self.axis, (int, np.integer)):
            raise TypeError("axis must be an integer")
        axis = int(self.axis)
        if axis not in (0, 1, 2):
            raise ScalarFieldError("axis must be 0, 1, or 2")
        if isinstance(self.index, bool) or not isinstance(self.index, (int, np.integer)):
            raise TypeError("index must be an integer")
        index = int(self.index)
        if index < 0 or index >= shape[axis]:
            raise ScalarFieldError(f"slice index {index} is outside source axis size {shape[axis]}")
        ...
        if not np.isfinite(fractional) or fractional != expected_fractional:
            raise ScalarFieldError("fractional_coordinate must equal index / source axis size")
        axes = tuple(int(value) for value in self.in_plane_axes)
        expected_axes = tuple(value for value in range(3) if value != axis)
        if axes != expected_axes:
            raise ScalarFieldError("in_plane_axes must preserve the remaining source-axis order")
        values = _frozen_float_array(self.values, name="values", ndim=2)
        expected_shape = tuple(shape[value] for value in axes)
        if values.shape != expected_shape:
            raise ScalarFieldError("slice values shape must match the exact retained source-grid plane")
        digest = hashlib.sha256()
        digest.update(b"CatalysisWorkbench.ScalarFieldSlice.v1\0")
        for text in (source_field_digest, structure_digest, field_kind, value_unit, registration_id):
            _digest_text(digest, text)
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, ScalarFieldSlice) and self.digest == other.digest and np.array_equal(self.values, other.values)


def scalar_field_from_volumetric_grid(grid: VolumetricGrid, component: str, *, field_kind: str, registration_id: str | None = None, source_key: str | None = None, metadata: Mapping[str, Any] | None = None) -> ScalarField:
    ...
    if retained_component not in grid.components:
        raise ScalarFieldError(f"component {retained_component!r} is not retained by VolumetricGrid")
    key = f"volumetric:{retained_component}" if source_key is None else str(_nonblank(source_key, name="source_key"))
    provenance = {"volumetric_grid_digest": grid.digest, "component": retained_component}
    if metadata:
        provenance.update(dict(metadata))
    return ScalarField(structure=grid.structure, values=grid.components[retained_component], field_kind=field_kind, value_unit=grid.density_unit, source_type="VolumetricGrid", source_key=key, source_digest=grid.digest, registration_id=registration_id, metadata=provenance)


def scalar_field_from_charge_density_difference(result: ChargeDensityDifferenceResult, *, field_kind: str = "charge-density-difference", source_key: str = "charge-density-difference", metadata: Mapping[str, Any] | None = None) -> ScalarField:
    if not isinstance(result, ChargeDensityDifferenceResult):
        raise TypeError("result must be a ChargeDensityDifferenceResult")
    provenance = {"result_digest": result.digest, "difference_grid_digest": result.difference_grid.digest, "source_component": result.source_component}
    if metadata:
        provenance.update(dict(metadata))
    return ScalarField(structure=result.difference_grid.structure, values=result.difference, field_kind=field_kind, value_unit=result.density_unit, source_type="ChargeDensityDifferenceResult", source_key=source_key, source_digest=result.digest, registration_id=result.registration_id, metadata=provenance)


def slice_scalar_field(scalar_field: ScalarField, *, axis: int, index: int) -> ScalarFieldSlice:
    ...
    if retained_index < 0 or retained_index >= size:
        raise ScalarFieldError(f"slice index {retained_index} is outside source axis size {size}")
    ...
    return ScalarFieldSlice(source_field_digest=scalar_field.digest, structure_digest=scalar_field.structure.digest, lattice_angstrom=lattice, grid_shape=scalar_field.grid_shape, field_kind=scalar_field.field_kind, value_unit=scalar_field.value_unit, registration_id=scalar_field.registration_id, axis=retained_axis, index=retained_index, fractional_coordinate=retained_index / size, values=values, in_plane_axes=(axes[0], axes[1]))


def fractional_grid_coordinate(scalar_field: ScalarField, index: Sequence[int]) -> NDArray[np.float64]:
    if not isinstance(scalar_field, ScalarField):
        raise TypeError("scalar_field must be a ScalarField")
    retained = _grid_index(index, grid_shape=scalar_field.grid_shape)
    result = np.asarray([retained[axis] / scalar_field.grid_shape[axis] for axis in range(3)], dtype=np.float64)
    result.setflags(write=False)
    return result


def cartesian_grid_coordinate(scalar_field: ScalarField, index: Sequence[int]) -> NDArray[np.float64]:
...
def slice_fractional_coordinate_grid(scalar_slice: ScalarFieldSlice) -> NDArray[np.float64]:
    ...
    coordinate[..., first_axis] = first_indices / scalar_slice.grid_shape[first_axis]
    coordinate[..., second_axis] = second_indices / scalar_slice.grid_shape[second_axis]
    coordinate.setflags(write=False)
    return coordinate


def slice_cartesian_coordinate_grid(scalar_slice: ScalarFieldSlice) -> NDArray[np.float64]:
    fractional = slice_fractional_coordinate_grid(scalar_slice)
    result = np.asarray(fractional @ scalar_slice.lattice_angstrom, dtype=np.float64)
    result.setflags(write=False)
    return result


__all__ = ["ScalarField", "ScalarFieldError", "ScalarFieldSlice", "cartesian_grid_coordinate", "fractional_grid_coordinate", "scalar_field_from_charge_density_difference", "scalar_field_from_volumetric_grid", "slice_cartesian_coordinate_grid", "slice_fractional_coordinate_grid", "slice_scalar_field"]
